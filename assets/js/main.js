...
(function () {
  ...
  /* ---------- Gold pixel sprite cursor ---------- */
  (function () {
    var fine = window.matchMedia('(hover: hover) and (pointer: fine)');
    if (!fine.matches) return;
    var reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    document.documentElement.classList.add('css-sprite-cursor');
    if (reduce) return;
    ...
    el.id = 'sprite-cur';
    el.innerHTML =
      '<img class="c-arrow" src="/assets/img/cursors/arrow-48.png" alt="" width="48" height="48">' +
      '<img class="c-hand" src="/assets/img/cursors/pointer-48.png" alt="" width="48" height="48">';
    document.body.appendChild(el);
    root.classList.add('sprite-cur');

    var HOT = 6;
    var HIT = 'a,button,summary,select,label,input,textarea,[role="button"],.press,.coin-cta,.item,.gframe,.cbtn,.hud-menu-btn,.tn-chev';
    var x = 0, y = 0, on = false;

    function move(e) {
      x = e.clientX; y = e.clientY;
      el.style.transform = 'translate(' + (x - HOT) + 'px,' + (y - HOT) + 'px)';
      var hit = e.target && e.target.closest && e.target.closest(HIT);
      if (!!hit !== on) {
        on = !!hit;
    ...
    document.addEventListener('pointerup', function () { el.classList.remove('punch'); });
    document.addEventListener('pointerleave', function () { el.style.opacity = '0'; });
    document.addEventListener('pointerenter', function () { el.style.opacity = '1'; });
  })();
})();
